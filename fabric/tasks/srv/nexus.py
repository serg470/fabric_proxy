...
def provision():
    ...
    nexusURI = "https://nexus.welespay.ru/service/rest/v1/script"
    ...
    i3 = "security.addUser('writer', 'writer', 'GITLAB-CI', 'root@localhost', true, '" + getpass.getpass(
    ...
    i4 = "security.securitySystem.changePassword('admin', '" + getpass.getpass('Admin password') + "')"
    data = {
        "name": "a4",
        "type": "groovy",
        "content": i4
    ...
    r = requests.post(nexusURI, auth=('admin', 'admin123'), verify=False, headers={"Content-Type": "application/json"},
                      data=json.dumps(data))
    print(r.status_code)
    r = requests.post(nexusURI + "/a4/run", auth=('admin', 'admin123'), verify=False,
                      headers={"Content-Type": "text/plain"})
    print(r.status_code)
...
def scrub(image, min_version):
    ...
    dockerURI = "https://docker-vpn.welespay.ru/"
...
def deploy():
    ...
    sudo("mkdir -p /mirror/nexus/data && chown -R 200 /mirror/nexus/data")
